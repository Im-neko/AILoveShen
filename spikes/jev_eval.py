"""Jev spike: let Jev pick one of the currently available bounded actions.

Compares raw vs summarized state and optionally runs a closed loop (observe -> decide -> act).
...
SCENARIOS = {
    ...
    "drop_nearby": ([f'execute at {BOT} run summon item ~5 ~1 ~ {{Item:{{id:"minecraft:iron_ingot",count:3}}}}'], 2,
                    {"pickup_drop"}),
    "hungry_safe": (["sleep 2", f"effect give {BOT} hunger 30 255", "sleep 10", f"effect clear {BOT}",
                     f"give {BOT} bread 3"], 1, {"eat"}),
    # Harder calls: no single obviously-correct answer from the action text alone.
    "hungry_zombie_near": (["time set night", "sleep 2", f"effect give {BOT} hunger 30 255", "sleep 10",
                            f"effect clear {BOT}", f"give {BOT} bread 3",
                            f"execute at {BOT} run summon zombie ~3 ~ ~ {{NoAI:1b}}"], 1, {"flee_hostile", "attack_hostile"}),
    "zombie_far_day": ([f"give {BOT} iron_sword", f"execute at {BOT} run summon zombie ~14 ~ ~ {{NoAI:1b}}"], 2,
                       {"collect_log", "attack_hostile"}),
    "unarmed_full_zombie": (["time set night", f"execute at {BOT} run summon zombie ~4 ~ ~ {{NoAI:1b}}"], 2,
                            {"attack_hostile", "flee_hostile"}),
    "night_zombie_armed": (["time set night", f"give {BOT} iron_sword",
                            f"execute at {BOT} run summon zombie ~4 ~ ~ {{NoAI:1b}}"], 2,
                           {"attack_hostile", "equip_weapon"}),
    "night_zombie_lowhp": (["time set night", f"damage {BOT} 14",
                            f"execute at {BOT} run summon zombie ~3 ~ ~ {{NoAI:1b}}"], 2, {"flee_hostile"}),
    "creeper_close": ([f"execute at {BOT} run summon creeper ~3 ~ ~ {{NoAI:1b}}"], 2, {"flee_hostile"}),
}


def rcon(cmd: str) -> str:
    return subprocess.run(["docker", "exec", "ailoveshen-minecraft", "rcon-cli", cmd],
...
async def decide(client: AsyncTypeSafeClient, state: dict, actions: list[dict], key: str = "description") -> dict:
    criteria = {a["id"]: a[key] for a in actions}
    t = time.perf_counter()
    r = await client.system_one(state=state, questions={"action": Choice(instructions=INSTRUCTIONS, criteria=criteria)})
...
# variant -> (state key, criteria description key)
VARIANTS = {
...
async def compare(repeat: int, label: str, scenario: str | None = None) -> None:
    ...
    out = {"label": label, "expected": expected, "summary": obs["summary"], "actions": obs["actions"], "runs": {}}
    print(f"== {label}: {json.dumps(obs['summary'], ensure_ascii=False)[:400]}")
    print("actions:", [a["id"] for a in obs["actions"]])
...
async def loop(steps: int) -> None:
    async with AsyncTypeSafeClient() as client:
        for i in range(steps):
            ...
            s = obs["summary"]["self"]
            print(f"[{i}] hp={s['health']} food={s['food']} phase={obs['summary']['time']['phase']} "
                  f"-> {d['choice']} (conf {d['confidence']}, {d['latency_s']}s)", flush=True)
            res = act(d["choice"])
