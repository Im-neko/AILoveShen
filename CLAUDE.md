...
- **Three-layer LLM → Jev → Minecraft Bridge link** (`GameService.play`; design: `docs/design/10_agent_lifecycle.md`, `11_primitive_actions.md`):
  ...
  - Exploring: each direction is offered up to where unseen land begins (`memory.frontierDistance` over explored 16x16 regions) and walked in legs, so a search with nothing nearby does not circle the same area; logs are reachable down through air and logs to the first ground (5 blocks, 8 with dirt to tower)
