...
- **Three-layer LLM → Jev → Minecraft Bridge link** (`GameService.play`; design: `docs/design/10_agent_lifecycle.md`, `11_primitive_actions.md`):
  ...
  - Stuck walking (`minecraft-bridge/src/move.mjs`, `walkTo`): when the bot moves < 0.5 m for 6 s while walking (not digging or placing), it jumps out, then walks carefully (no sprint or parkour), then backs off a few blocks, before failing with `stuck: …`
