...
- **Jev first** (`docs/design/34_jev_first.md`): in tools mode each step first goes to Jev (`StepPicker`, `minecraft.agent.step_picker: jev`): it picks among the bridge's candidates (run as `do_suggestion`), learned skills that worked and take no arguments (`run_skill`) or `ask_gemini`; Gemini's `choose_tool` decides when Jev asks, is unsure (< `step_min_confidence`), slow, fails, after a failed tool, after `step_max_failures` failed Jev steps or every `step_gemini_every` steps; records in `logs/steps/*.jsonl`
