...
- **Three-layer LLM → Jev → Minecraft Bridge link** (`GameService.play`; design: `docs/design/10_agent_lifecycle.md`, `11_primitive_actions.md`):
  ...
  - Planting and farming (`docs/design/33_planting_and_farming.md`, bridge `farming.mjs`): `planted(sapling|kind, n)` counts saplings or grown trees where the bot planted (`state.plantings`), `farmed(crop, n)` crops on farmland near the home (wheat / carrots / potatoes / beetroots); both are conditions and skill expects. Tools and primitives `plant` / `till` / `sow` (the bridge picks a spot 6–24 m from the home when none is given; never on or next to the home or builds); `dig` also takes grass, crops and saplings. Saplings come from leaves and seeds from grass (chance drops: nothing dropped is not a failure). The procedure is left to Gemini's skills in tools mode; candidates mode has the basic plant / sow / till steps
  - Gathering: `stored(item, n)` carries items home in batches (deposit only at 32 carried, enough to finish, a nearly full inventory, near the home, nothing left to gather here, or dusk/night), and every goal is sent with `also` (items the other mid goals and the current steps gather: have / stored / planted → saplings / farmed → seeds): gather candidates for them within 8 m are offered by day, marked `also` (a tree gives logs and, from its leaves, saplings); such side work is not counted as a stall
  - Food: cooked food is eaten first; raw meat only when starving (food ≤ 6) or when nothing can cook it (≤ 12); raw meat is cooked at a visible furnace or walked to the last seen one (`state.lastFurnace`, 3+ pieces). Chance-drop digging (grass for seeds, leaves for saplings) sweeps the same kind within 5 m in one action until something drops, and digs plants within reach without walking
  - Night (`through_night`, no home needed): inside the home, sleep in its bed; outside, go home, sleep in a bed within 32 m that is not the home's (`sleep_at`, e.g. a village), place a carried bed and sleep (`bed_here`, the bed stays and sets the spawn), or stay underground (`observation.underground`: covered and below y 60; the survival goals for night/dusk are not offered there). Jev picks among them
  - Tool wear (`minecraft-bridge/src/wear.mjs`): a tool that disappears at its last durability is recorded as broken at once (`state.brokenTools`, `observation.broken_tools`, shown as 「壊れた道具」, and appended to the action's result); while no tool of that kind is held, crafting another is offered (`also`) when it is craft-only; blocks that need a tool are never dug bare-handed (the dig fails with the reason). Chat replies re-read `/observe` first (`GameService.refresh`)
  - Stuck walking (`minecraft-bridge/src/move.mjs`, `walkTo`): when the bot moves < 0.5 m for 6 s while walking (not digging or placing), it jumps out, then walks carefully (no sprint or parkour), then backs off a few blocks, before failing with `stuck: …` (digging/placing counts only after 30 s); a block the pathfinder digs 3 times in a minute (dig → place dirt → dig loop) is neither dug nor placed for 3 minutes (`guardDigLoops`, `state.loopCells`)
  - Exploring: each direction is offered up to where unseen land begins (`memory.frontierDistance` over explored 16x16 regions) and walked in legs, so a search with nothing nearby does not circle the same area; logs are reachable down through air and logs to the first ground (5 blocks, 8 with dirt to tower); digging stairs down to buried stone/ore has no depth limit by default (`dig_depth` is an optional cap, 0 = do not dig down; lava, water, gravel and bedrock stop it)
...
- **Tool control** (`minecraft.agent.control: tools`, the default since 2026-09-26, `docs/design/19_gemini_tools.md`, `21_tool_control.md`; `candidates` (the path above, Jev picks the bridge's candidate) is kept for comparison with `--control candidates`): within the same predicate small goals, Gemini calls one bridge tool per step by function calling (`POST /tool`: goto/dig/place/craft/…, `do_suggestion` runs a solver candidate, `find_blocks`/`recipe_of`/`how_to_get` are lookups). Action tools carry an `intent` (shown via `activity()`) and up to 3 English `watch` questions. While a tool runs, `ToolWatcher` reads `GET /state` (progress, a `look_around` height grid, mobs with ids, needs) every second and asks Jev all questions in one `system_one` call; two strong yeses stop / wake / report maybe-done via `POST /abort` (Jev never judges completion; the code's own progress question only records until it beats a rule). Safety stays in the bridge: sheltering refuses outside tools, the current home and the house being built are never dug (former homes can be). Ticks go to `logs/watch/*.jsonl`. Thinking depth per purpose: `gemini.thinking_levels`
- **Jev first** (`docs/design/34_jev_first.md`): in tools mode each step first goes to Jev (`StepPicker`, `minecraft.agent.step_picker: jev`): it picks among the bridge's candidates (run as `do_suggestion`), learned skills that worked and take no arguments (`run_skill`) or `ask_gemini`; Gemini's `choose_tool` decides when Jev asks, is unsure (< `step_min_confidence`), slow, fails, after a failed tool, after `step_max_failures` failed Jev steps or every `step_gemini_every` steps; records in `logs/steps/*.jsonl`. The voice's emotion (`tts.emotion_judge: jev`): before a line is spoken, one `AvatarDirector.react` call picks the expression, strength and gesture; `voice_emotion` turns it into the line's `EmotionState` (`TTSService.speak(emotion=)`: Style-Bert-VITS2 style, Irodori-TTS emotion voice/caption) and the avatar reuses the same reaction (`LineReactions`); no answer → neutral as before. Chat triage (`twitch.response.triage: jev`, `CommentTriage`): before a reply Jev answers reply-or-not and the kind (request / advice / withdraw / question / chat / noise); requests, advice and withdrawals always get Gemini's reply; only chat / noise Jev is sure needs no reply is skipped (`logs/chat/*.jsonl`); withdrawals and requests are answered first. Narration timing (`stream.commentary_judge: jev`, `CommentaryGate`): at a small-goal boundary Jev says whether it is worth speaking now; highlights (mid goal completed / dropped, house, town) and `max_silence_seconds` (60) of quiet always speak, skipped events wait for the next line (`logs/commentary/*.jsonl`). Failure routing (`minecraft.agent.failure_routing: jev`): after a stuck / stalled small goal, `GoalChooser.choose(avoid=)` offers Jev the other open steps (not the same kind) plus `rethink` (Gemini's design-27 analysis); Gemini always after 2 same-kind failures, a death, a stalled mid goal or a screen rethink. Stuck check (`minecraft.agent.stuck_check_steps`, 6, `StuckCheck`): every 6 actions of a small goal Jev reads the action record (actions, results, positions) and picks fine / repeating / failing / back_and_forth / no_progress; a sure non-fine answer (≥ 0.75) calls `request_rethink` and Gemini re-decides with the action record (`logs/stuck/*.jsonl`)
...
- **Mid goal not progressing** (`minecraft.agent.mid_goal_stall_steps`, 60): when the top mid goal's progress (the `(n/m)` counts of its conditions) stays the same over that many actions for it, the next boundary goes to Gemini (`goal_after_failure`, retry not offered) with the reason and alternatives (another method, grow what cannot be found: `planted` saplings when there are no trees, `farmed` for food; explore far; move down or drop), even when small goals keep ending as met or stalled and Jev would pick the same step again
- **Review each decision** (`docs/design/31_review_each_decision.md`): every Gemini goal decision writes a required `review` first (after diagnosis/remedy/advice on a failure): from the state now (inventory, chests, nearby, memory, time, the top mid goal's steps judged by `/check` and shown as 「手順の今の状態」) is there a faster or less wasteful way; it then edits the mid goals / rewrites the steps, or says why the plan stays (no change is a valid answer). `Goal.review` is shown in `activity()` and logged as 見直し
...
- **Coherent decisions** (`docs/design/12_coherent_decisions.md`, request flow replaced by 13): `PlaySession` alone owns the goals; its `activity()` (mission, mid goals, small goal and status, recent goals) is rendered by one formatter (`prompts/stream_context.py`) for the goal decision, the commentary and the chat replies, and the goal decision also sees the conversation. A chat reply and its handling of a request (none/accept/decline) come from one generation; a reply that agrees with a concrete, checkable point (a tree right there, a faster way) also writes `rethink`, which cuts the small goal at the next boundary (`request_rethink`, at most once a minute from chat) so the goal is decided again; `lesson` (advice the streamer agrees with, in its own words) is kept as a lesson note via `NoteKeeper.learn_from_viewer` (repeated advice extends it; design 18 addendum); `withdraw` (the requester calls off their own request) drops that viewer's mid goal (`MidGoalKeeper.withdraw`, only their own); an accepted request becomes a viewer's mid goal behind the current one (the small goal is not interrupted). `Narrator` says why goals change and never drops a promise silently
...
python -m ailoveshen.stream [--control candidates] [--no-speak] [--no-chat] [--no-board] [--debug]
