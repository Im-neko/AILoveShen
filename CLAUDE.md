...
- **Goal board** (`presentation/web/goal_board.py`, extra `ailoveshen[stream]`): `GET /api/goals`, `/api/goals/stream` (SSE), `/overlay` (OBS browser source), read-only in the play process (`examples/integration_test_minecraft.py --board-port 8765`). Debug: `GET /api/debug/gemini?limit=N` (the last Gemini calls, newest first: purpose, thinking level, thought summary with `gemini.include_thoughts`, output, tool calls, tokens, prompt; kept by `InMemoryGenerationLog`, shared by the game and chat generators) and `/debug/gemini` (a page that polls it; works as an OBS browser source)
