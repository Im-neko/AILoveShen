...
import { availableActions, configureMovements, findLog, findTable, EXECUTORS, ACTION_TIMEOUT_MS } from './actions.mjs'
...
const state = { plan: null, history: [], busy: false, recipeBook: new RecipeBook(bot), unreachableDrops: new Set() }
...
async function act (id) {
  ...
  const timer = setTimeout(() => abort('timeout'), ACTION_TIMEOUT_MS)
  const onHurt = (entity) => {
...
async function handle (req, res) {
  ...
  if (req.method === 'GET' && req.url === '/observe') {
    return send(res, 200, { busy: state.busy, observation: observation(), actions: availableActions(bot, state) })
  }
