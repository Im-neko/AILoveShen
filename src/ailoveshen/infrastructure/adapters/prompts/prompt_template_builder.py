...
- request は次のどれか
  ...
  - accept: 引き受ける。題名（title。頼んだ人の名前は入れない。別に記録されて表示に付く）、
    ...
    - when=now: 今の小目標が待っているだけ（夜明けを待つ through_night、家にいる at_home）で、
      その場（夜なら家の中）でできる頼みなら、待ち時間に今やる。リストの先頭に入り、終わったら
      今の中目標に戻る。返答では「今やるね」と言ってよい。夜の家の中でできるのは、持っている物からの
      クラフト（作業台は家の中に置ける）と、家の中に置くこと。外に出る頼み（木を切る、探検、戦い）は
      夜は now にしない。待っているのでなければ now にしない
