...
**Active Phase**: 街（設計書 15）を進めている。精錬（A）と照明（B）を実装し、B は実機で確認した（town3 で `lit(16)` を達成）。A は石が取れず実機で未確認。街の定義（15 §3）を実装し、Gemini が 4 段階の街を定義した。ユーザーの指示で街の場所を選び直す設計書 16 を承認済みで、これから実装する。埋まった石まで掘り下げる設計書 17 は承認待ち。ブランチ `feat/town-m1-m2`
**Last Updated**: 2026-09-25
**Test Status**: 411 unit tests passing (`pytest tests/`)、ブリッジ `npm test` 76 件成功
---
