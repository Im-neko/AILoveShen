...
- 19 §10: 洞窟の穴（town4c）、前の家のベッド（town4d）、何もない所から鉄の剣を A でたどった。A のままでは足りないもの: ブロックの形が観測にない、完了を確かめる言葉が閉じた述語と `HouseBlueprint` だけ（20 の自由な大目標もこれに縛られる）、Gemini のループの意図が `activity()` になく返答と食い違う。道具に隠れた決まり（前の家を掘らない、足場は土だけ、など）を安全と好みに分けた
...
0. **設計書 19 §13 の 9〜11 の返事**（A の範囲、夜の決まり、Jev の役の順番）。19 §13 の 1〜8 と 20 §8 は承認済み。承認後: 見張りの記録（`/progress`）と `/abort` → 考える深さ（用途ごとの thinking_level）→ §6 の評価（ユーザーの環境で）→ 見張りをループに → 設計書 21（A）。下の 2（低レベルの操作を Jev に）は、有界の低レベル操作を Jev ではなく A の道具として Gemini に渡すことで決着した（19 §13 の 8）
