...
**Active Phase**: 16（街の場所）の実機での確認中（town4d で調査 → 選択 → 定義 → 新しい家の完成 → 家の切り替えまで通った。引っ越しの中目標はベッドで止まっている）。18（Gemini のメモ）はブランチ `feat/notes` で作業中。ブランチ `feat/town-m1-m2`
**Last Updated**: 2026-09-25（続きはクラウドのセッションで検討する）
**Test Status**: `feat/town-m1-m2`: 435 unit tests（`pytest tests/`）、ブリッジ 96 件（`npm test`）。`feat/notes`: 454 件、ブリッジ 95 件
