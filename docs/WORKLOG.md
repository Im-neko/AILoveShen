...
**Active Phase**: 世界の記憶（`docs/design/14_world_memory.md`: 拠点の設計と名前、見た場所、チェスト）を 3 段階とも実装した（単体テストまで）。次はブリッジを再起動して stage 2・3 を実機で確認する。ブランチ `feat/town-m1-m2`（`feat/phase6-minimal` から派生、PR #19 は未マージ）
**Last Updated**: 2026-09-25
**Test Status**: 385 unit tests passing (`pytest tests/`)、ブリッジ `npm test` 44 件成功
...
- **14 の実機確認**: ブリッジを再起動して（ミラーの接続が切れるので事前に告知）、覚えた場所へ向かうか、チェストの設置・出し入れ・再起動後の記録、`stored` の中目標（14 §10）
- 起動時の判定で完了した中目標（前から建っていた家）を「完成したよ」と実況しない
- 進められない中目標（石炭のない松明）が、予算切れの断念か順番の入れ替えで後ろの中目標（ベッド）を塞ぎ続けないか確認する
