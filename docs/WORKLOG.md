...
**Active Phase**: 16（街の場所）と 17（掘り下げ）を実装し、新しいワールドで自律実行（town4c）をしている。18（Gemini のノート）は承認済みで、16 の実機が済んでから実装する（ユーザーの承認した順番）。視聴者の頼みの予算はユーザーの返事待ち。ブランチ `feat/town-m1-m2`
**Last Updated**: 2026-09-25
**Test Status**: 435 unit tests passing (`pytest tests/`)、ブリッジ `npm test` 93 件成功
---
...
- **設計書 16 の実装**（街の場所の調査と引っ越し）→ 自律実行で、調査 → 選択 → 定義のやり直し → 新しい家 → 家の切り替えまでを通す
- **設計書 17 の承認**（埋まった石まで階段を掘る）。石がないと鉄の剣・かまど・精錬（A）・肉を焼くことが止まる
...
- 視聴者の頼みの予算 80 ステップ（何もない所からの鉄の剣は正当に超える）: ユーザーの判断待ち
