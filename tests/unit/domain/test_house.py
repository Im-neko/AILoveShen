...
def _obs(inventory=None, actions=(), build=None, table=False) -> GameObservation:
...
class TestHouseProjectMaterialNeeds:
    ...
    def test_logs_in_hand_reduce_shortfall(self):
        ...
        needs = HouseProject(blueprint=_blueprint()).material_needs(_obs({"spruce_log": 20}))
    ...
    def test_table_nearby_or_held_is_not_needed(self):
        ...
        assert not project.material_needs(_obs({"crafting_table": 1})).table_needed
    ...
    def test_door_in_hand_needs_no_door_planks(self):
        ...
        needs = HouseProject(blueprint=_blueprint()).material_needs(_obs({"spruce_door": 1}))
    ...
    def test_remaining_blocks_come_from_build_status(self):
        ...
        obs = _obs({"oak_planks": 10}, build=_build({BlockKind.PLANKS: 10, BlockKind.DOOR: 1}))
    ...
    def test_corner_logs_count_as_logs(self):
        ...
        needs = HouseProject(blueprint=_blueprint(corner_pillars=True)).material_needs(
            _obs({"spruce_door": 1, "spruce_planks": 59})
        )
...
class TestHouseProjectGoals:
    ...
    def test_gather_wood_met_when_enough_logs(self):
        ...
        assert not project.goal_met(_obs({"spruce_log": 20}))
        assert project.goal_met(_obs({"spruce_log": 21}))
    ...
    def test_craft_met_when_planks_and_door_ready(self):
        ...
        assert not project.goal_met(_obs({"spruce_planks": 71}))
        assert project.goal_met(_obs({"spruce_planks": 71, "spruce_door": 1}))
    ...
    def test_explore_met_after_steps(self):
        ...
        project.set_goal(Goal(GoalType.EXPLORE))
