...
def _wav(seconds: float, amplitude: int = 8000) -> bytes:
    b = io.BytesIO()
    with wave.open(b, "wb") as w:
        w.setnchannels(1)
        w.setsampwidth(2)
        w.setframerate(8000)
        w.writeframes(b"".join(struct.pack("<h", int(amplitude * math.sin(i * 0.05))) for i in range(int(8000 * seconds))))
    return b.getvalue()


def test_wav_stats_reads_length_peak_and_loudness():
    seconds, peak, rms = tool.wav_stats(_wav(1.5))
...
def test_check_keeps_a_normal_reading_and_names_why_others_are_left_out():
    ...
    assert tool.check(line, *tool.wav_stats(_wav(1.2, amplitude=32767))) == "音割れ"
