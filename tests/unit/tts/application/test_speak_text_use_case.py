...
from ailoveshen.core.domain.value_objects import EmotionState, EmotionType, SpeechPriority
from ailoveshen.tts.application.dto.speech_dto import SpeakTextRequest
...
@pytest.fixture
def emotion_style_service():
    """Create emotion style service."""
...
class TestSpeakTextUseCase:
    ...
    @pytest.mark.asyncio
    async def test_execute_uses_emotion_based_style(
        self,
        mock_synthesizer,
        mock_audio_player,
        mock_event_publisher,
        ...
        """Test that emotion-based style is used when no style specified."""
        ...
        request = SpeakTextRequest(text="Hello", style=None)
        await use_case.execute(request)

        # Should use "Happy" style based on emotion
        mock_synthesizer.synthesize.assert_called_once()
        call_kwargs = mock_synthesizer.synthesize.call_args.kwargs
        assert call_kwargs["style"] == "Happy"

    @pytest.mark.asyncio
    async def test_execute_uses_override_style(self, use_case, mock_synthesizer):
        """Test that override style is used when specified."""
        request = SpeakTextRequest(text="Hello", style="Sad")
